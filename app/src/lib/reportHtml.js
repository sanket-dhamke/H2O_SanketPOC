...
export function buildSchoolReportHtml(report) {
  ...
  const filename = `${brand.name}-${esc(report.societyName).replace(/[^A-Za-z0-9]/g, "")}-school-report`;
...
export function buildWingReportHtml(report) {
  ...
  const filename = `${brand.name}-${esc(society?.name).replace(/[^A-Za-z0-9]/g, "")}-${String(wing).replace(/[^A-Za-z0-9]/g, "")}-report`;
...
<body>
  <div class="wrap">
    ...
    <div class="footer">System-generated by ${brand.name}. For society records & backup.</div>
  </div>
</body>
</html>`;
