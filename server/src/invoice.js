...
export async function emailPremiumInvoice(societyId, { amount, expiresAt, note } = {}) {
  ...
  const invoiceNo = "GATEZO-INV-" + society.id.replace(/[^a-z0-9]/gi, "").slice(-6).toUpperCase() + "-" + new Date().getFullYear();

  const text =
    `GATEZO Premium subscription invoice\n\n` +
    `Invoice: ${invoiceNo}\nSociety: ${society.name}\n` +
    `Plan: GATEZO Premium (yearly)\nAmount: ${money(amt)}\nValid until: ${fmtDate(exp)}\n` +
    (note ? `Note: ${note}\n` : "") +
    `\nThank you for using GATEZO.`;
  ...
  <div style="max-width:600px;margin:0 auto;border:1px solid #E1E8EC;border-radius:14px;overflow:hidden;background:#fff">
    <div style="background:#0B6E8F;color:#fff;padding:22px 26px;display:flex;justify-content:space-between">
      <div><div style="font-size:20px;font-weight:800">GATEZO Premium</div><div style="color:#CDE9F2;font-size:12px;margin-top:4px">Subscription invoice</div></div>
    ...
    <div style="padding:24px 26px">
      ...
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:10px 0;border-top:1px solid #EEF2F4;color:#6B7B85">GATEZO Premium plan (1 year)</td><td style="padding:10px 0;border-top:1px solid #EEF2F4;text-align:right;font-weight:700">${money(amt)}</td></tr>
      ...
      <p style="margin-top:20px;color:#8895A0;font-size:12px">Premium unlocks the vendor venue marketplace, voice AI and more. Thank you for using GATEZO.</p>
    </div>
  </div></body></html>`;
  ...
  for (const to of recipients) {
    const r = await sendEmail({ to, subject: `GATEZO Premium invoice — ${society.name}`, text, html });
