...
function buildReceiptPdf({ bill, resident, society, account }) {
  return new Promise((resolve, reject) => {
    ...
    const societyName = society?.name || account?.accountHolderName || "GATEZO Society";
...
export async function onBillPaid(billId) {
  try {
    ...
    if (resident?.email) {
      const text = `Hi ${resident.name},\n\nThank you! We received your maintenance payment of ${inr(bill.amount)} for ${bill.period} (Flat ${flatNo}). Your receipt is attached.\n\n— ${society?.name || "GATEZO Society"}`;
      const html = `<p>Hi ${resident.name},</p><p>Thank you! We received your maintenance payment of <b>${inr(bill.amount)}</b> for <b>${bill.period}</b> (Flat ${flatNo}). Your receipt is attached as a PDF.</p><p>— ${society?.name || "GATEZO Society"}</p>`;
      await sendEmail({
        to: resident.email,
